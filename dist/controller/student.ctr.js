...
exports.monthsInUzbek = exports.getMonthlyStudentStats = void 0;
...
exports.getTodayAttendanceStats = getTodayAttendanceStats;
exports.getAttendanceByDate = getAttendanceByDate;
exports.updateAttendance = updateAttendance;
exports.extendAttendanceTime = extendAttendanceTime;
exports.getExtendAttendanceTime = getExtendAttendanceTime;
exports.generateStudentId = generateStudentId;
exports.getGroupAttendanceSummary = getGroupAttendanceSummary;
exports.getOverallAttendanceStats = getOverallAttendanceStats;
...
const extend_attendance_model_1 = require("../Models/extend_attendance_model");
...
exports.monthsInUzbek = monthsInUzbek;
...
async function getStudents(req, res, next) {
    try {
        ...
        const studentsWithGroups = students.map((student) => {
            ...
            const paidGroups = monthStudentGroups.filter((sg) => sg.paid).length;
            return {
                ...plain,
...
async function getOneGroupStudentsForTeacher(req, res, next) {
    try {
        const lang = "uz";
        const groupId = req.query.group_id;
        if (!groupId)
...
async function createStudent(req, res, next) {
    try {
        ...
        if (!branchId) {
            return next(base_error_1.BaseError.BadRequest(403, "Branch aniqlanmadi"));
        }
        const branchManager = await index_1.User.findOne({ where: { branch_id: branchId, role: "manager" } });
        const lang = req.headers["accept-language"]?.split(",")[0] || "uz";
        ...
        try {
            ...
            student = await index_1.Student.create({
                first_name,
                last_name,
                father_name,
                mother_name,
            ...
            for (const group_id of group_ids) {
                ...
                const group = await index_3.Group.findOne({
                    where: (0, branch_scope_helper_1.withBranchScope)(req, { id: group_id }),
                    transaction: t,
                });
                if (group) {
            ...
            if (notificationsPayload.length > 0) {
                createdNotifications = await index_1.UserNotification.bulkCreate(notificationsPayload, {
                    transaction: t,
                    returning: true,
                });
            }
    ...
    try {
        ...
        try {
            ...
            if (group_ids && Array.isArray(group_ids)) {
                const existingStudentGroups = await student_groups_model_1.default.findAll({
                ...
                for (const groupId of removedGroupIds) {
                    const group = await index_3.Group.findByPk(groupId, { transaction: t });
                    if (group) {
                        await group.increment("students_amount", { by: -1, transaction: t });
                    }
                    await student_groups_model_1.default.destroy({
                ...
                for (const group_id of newGroupIds) {
                    ...
                    const group = await index_3.Group.findByPk(group_id, { transaction: t });
                    if (group) {
                        await group.increment("students_amount", { by: 1, transaction: t });
                    }
                }
                // total_groups ni yangilash
                await student.update({ total_groups: group_ids.length }, { transaction: t });
            }
            await t.commit();
            await updateStudentPaymentStatus(student.dataValues.id);
            res.status(200).json(student);
...
async function deleteStudent(req, res, next) {
    try {
        ...
        try {
            ...
            const studentGroups = await student_groups_model_1.default.findAll({
                where: { student_id: student.dataValues.id },
                transaction: t,
            });
            for (const sg of studentGroups) {
                const group = await index_3.Group.findByPk(sg.dataValues.group_id, {
                    transaction: t,
                });
                if (group) {
                    await group.update({ students_amount: group.dataValues.students_amount - 1 }, { transaction: t });
                }
                await sg.destroy({ transaction: t });
            }
            await index_1.Notification.destroy({
...
const getMonthlyStudentStats = async (req, res, next) => {
    try {
        ...
        const totalTeachers = await index_2.Teacher.count({ where: (0, branch_scope_helper_1.withBranchScope)(req) });
        const totalGroups = await index_3.Group.count({ where: (0, branch_scope_helper_1.withBranchScope)(req) });
        ...
        const totalStudents = await index_1.Student.findAndCountAll({ where: (0, branch_scope_helper_1.withBranchScope)(req) });
        const totalPaymentThisMonth = await (0, payments_ctr_1.getThisMonthTotalPayments)(req);
        const latestPaymentsForThisMonth = await (0, payments_ctr_1.latestPayments)(req);
        const roomsBusinessPercentAll = await (0, room_ctr_1.getRoomsBusinessPercent)(branchIds);
        const studentsByGender = await index_1.Student.findAll({
            where: (0, branch_scope_helper_1.withBranchScope)(req),
            attributes: [
                [
                    sequelize_1.Sequelize.literal(`
...
async function updateStudentPaymentStatus(studentId, transaction) {
    try {
        ...
        const paidGroupsCount = await student_groups_model_1.default.count({
            where: {
...
async function getPaymentsByStudent(req, res, next) {
    ...
    try {
        const payments = await index_1.Payment.findAll({
            where: { ...(0, branch_scope_helper_1.withBranchScope)(req), pupil_id: student_id },
            order: [['created_at', 'DESC']],
            attributes: [
                'id',
                'pupil_id',
...
;
async function getGroupAttendanceSummary(req, res, next) {
    try {
        ...
        const classDays = group.dataValues.days.split('-').map((d) => d.trim().toUpperCase());
        // Haftalik statistika
        const weekStart = now.minus({ days: 6 }).startOf('day');
        const weekEnd = now.endOf('day');
        const weekStats = await Models_1.AttendanceRecord.findAll({
            attributes: [
                [sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.col('AttendanceRecord.id')), 'total'],
                [
                    sequelize_1.Sequelize.fn('SUM', sequelize_1.Sequelize.literal(`CASE WHEN "AttendanceRecord"."status" = 'present' THEN 1 ELSE 0 END`)),
                    'present'
                ],
            ],
            include: [{
                    ...
                    where: {
                        group_id: groupId,
                        date: { [sequelize_1.Op.between]: [weekStart.toISODate(), weekEnd.toISODate()] }
                    },
                    required: true,
        ...
        const monthStats = await Models_1.AttendanceRecord.findAll({
            attributes: [
                [sequelize_1.Sequelize.fn('COUNT', sequelize_1.Sequelize.col('AttendanceRecord.id')), 'total'],
                [
                    sequelize_1.Sequelize.fn('SUM', sequelize_1.Sequelize.literal(`CASE WHEN "AttendanceRecord"."status" = 'present' THEN 1 ELSE 0 END`)),
            ...
            include: [{
                    ...
                    where: {
                        group_id: groupId,
                        date: { [sequelize_1.Op.between]: [monthStart.toISODate(), monthEnd.toISODate()] }
                    },
                    required: true,
                    attributes: ['id', 'date'],
                }],
...
async function makeAttendance(req, res, next) {
    try {
        ...
        if (!records || !Array.isArray(records) || !date) {
            return next(base_error_1.BaseError.BadRequest(400, "Noto'g'ri formatdagi ma'lumot kiritildi"));
        }
        // Validate group
        const foundGroup = await index_3.Group.findByPk(group_id);
        if (!foundGroup) {
            return next(base_error_1.BaseError.BadRequest(404, "Guruh topilmadi"));
        }
        // Validate date
        const inputDate = luxon_1.DateTime.fromISO(date, { zone: "Asia/Tashkent" });
        const today = luxon_1.DateTime.now().setZone("Asia/Tashkent");
        if (inputDate > today.endOf("day")) {
            return next(base_error_1.BaseError.BadRequest(400, "Hali kelmagan sana kiritildi"));
        }
        ...
        if (existing) {
            return next(base_error_1.BaseError.BadRequest(400, "Yo'qlama qilingan"));
        }
        ...
        const extension = await extend_attendance_model_1.AttendanceExtension.findOne({
            where: {
                group_id,
                extended_until: { [sequelize_1.Op.gt]: now.toISO() }
            },
        ...
        if (!extension) {
            ...
            if (now < classStart) {
                ...
                return next(base_error_1.BaseError.BadRequest(400, `Dars boshlanishiga hali ${hours} soat ${minutes} daqiqa bor. Yo'qlama qilish mumkin emas.`));
            }
            if (now > classEnd) {
        ...
        try {
            ...
            for (const item of records) {
                ...
                if (!student_id || typeof student_id !== "string") {
                    throw base_error_1.BaseError.BadRequest(400, "Noto'g'ri o'quvchi ID'si kiritildi");
                }
                const foundStudent = await index_1.Student.findByPk(student_id, { transaction: t });
                if (!foundStudent) {
                    throw base_error_1.BaseError.BadRequest(404, "O'quvchi topilmadi");
                }
                if (!["present", "absent"].includes(status)) {
                    throw base_error_1.BaseError.BadRequest(400, "Yo'qlama uchun noto'g'ri status kiritildi");
                }
                if (status === "absent" && reason && !["excused", "unexcused"].includes(reason)) {
                    throw base_error_1.BaseError.BadRequest(400, "Yo'qlama uchun noto'g'ri sabab kiritildi");
                }
...
async function updateAttendance(req, res, next) {
    try {
        ...
        const attendance = await Models_1.Attendance.findOne({
            where: { group_id, date },
        });
        if (!attendance) {
            return next(base_error_1.BaseError.BadRequest(404, "Yo'qlama topilmadi"));
        }
        ...
        const [endHours, endMinutes] = foundGroup.dataValues.end_time.split(":").map(Number);
        const classEnd = luxon_1.DateTime.fromObject({ year: inputDate.year, month: inputDate.month, day: inputDate.day, hour: endHours, minute: endMinutes }, { zone: "Asia/Tashkent" }).plus({ hours: 1 });
        ...
        if (!extension && now > classEnd) {
            return next(base_error_1.BaseError.BadRequest(400, "Yo'qlama vaqti tugagan. Yangilash mumkin emas."));
        }
        const t = await database_config_1.default.transaction();
        try {
            for (const item of records) {
                const { student_id, status, reason, note } = item;
                if (!["present", "absent"].includes(status)) {
                    throw base_error_1.BaseError.BadRequest(400, "Status noto'g'ri");
                }
                const existingRecord = await Models_1.AttendanceRecord.findOne({
                    where: {
                        attendance_id: attendance.dataValues.id,
                        student_id,
                    },
...
async function extendAttendanceTime(req, res, next) {
    try {
        const { group_id, extended_until } = req.body;
        if (!group_id || !extended_until) {
            return next(base_error_1.BaseError.BadRequest(400, "Guruh ID va uzaytirish vaqti kerak"));
        }
        ...
        // Vaqtni tekshirish
        const extensionDate = luxon_1.DateTime.fromISO(extended_until, { zone: "Asia/Tashkent" });
        const now = luxon_1.DateTime.now().setZone("Asia/Tashkent");
        if (extensionDate <= now) {
            return next(base_error_1.BaseError.BadRequest(400, "Uzaytirish vaqti hozirgi vaqtdan keyin bo'lishi kerak"));
        }
        // Uzaytirishni yaratish yoki yangilash
        const [extension, created] = await extend_attendance_model_1.AttendanceExtension.findOrCreate({
            where: { group_id },
            defaults: {
                group_id,
...
async function getExtendAttendanceTime(req, res, next) {
    try {
        const group_id = req.params.groupId;
        const extension = await extend_attendance_model_1.AttendanceExtension.findOne({
            where: { group_id },
        ...
        if (!extension) {
            return next(base_error_1.BaseError.BadRequest(404, "Uzaytirish topilmadi"));
        }
...
async function getAttendanceByDate(req, res, next) {
    try {
        ...
        if (!date || typeof date !== "string") {
            return next(base_error_1.BaseError.BadRequest(400, "Noto'g'ri sana kiritildi"));
        }
        const attendance = await Models_1.Attendance.findOne({
            where: { group_id, date },
            include: [
                {
                    model: Models_1.AttendanceRecord,
                    as: "records",
                    include: [
                        {
...
async function getTodayAttendanceStats(req, res, next) {
    try {
        const lang = "uz";
        const groupTimeZone = "Asia/Tashkent";
        // Hozirgi vaqt Luxon bilan
        const now = luxon_1.DateTime.now().setZone(groupTimeZone);
        ...
        const groups = await index_3.Group.findAll({
            where: (0, branch_scope_helper_1.withBranchScope)(req),
        });
        // Bugungi darslar uchun guruhlarni filtr qilish
        const todaysGroups = groups.filter((g) => g.dataValues.days
            .split("-")
            .map((d) => d.trim())
        ...
        const stats = await Models_1.AttendanceRecord.findAll({
            attributes: [
                [
                    sequelize_1.Sequelize.fn("SUM", sequelize_1.Sequelize.literal(`CASE WHEN "AttendanceRecord"."status" = 'present' THEN 1 ELSE 0 END`)),
                    "present",
                ...
                [
                    sequelize_1.Sequelize.fn("SUM", sequelize_1.Sequelize.literal(`CASE WHEN "AttendanceRecord"."status" = 'absent' THEN 1 ELSE 0 END`)),
                    "absent",
                ],
                [sequelize_1.Sequelize.fn("COUNT", sequelize_1.Sequelize.col("AttendanceRecord.id")), "total"],
            ],
            include: [
                {
                    ...
                    required: true,
                    where: {
                        group_id: { [sequelize_1.Op.in]: todaysGroups.map((g) => g.dataValues.id) },
                        date: todayStr,
                    },
                },
            ],
...
async function getOverallAttendanceStats(req, res, next) {
    try {
        const now = luxon_1.DateTime.now().setZone("Asia/Tashkent");
        // Hafta
        const weekStart = now.minus({ days: 6 }).startOf('day');
        const weekEnd = now.endOf('day');
        const weekStats = await Models_1.AttendanceRecord.findOne({
            attributes: [
            ...
            include: [{
                    model: Models_1.Attendance,
                    as: 'attendance',
                    where: { date: { [sequelize_1.Op.between]: [weekStart.toISODate(), weekEnd.toISODate()] } },
                    include: [{
                            model: index_3.Group,
        ...
        const monthStart = now.startOf('month');
        const monthEnd = now.endOf('month');
        const monthStats = await Models_1.AttendanceRecord.findOne({
            attributes: [
                [(0, sequelize_1.fn)('SUM', (0, sequelize_1.literal)(`CASE WHEN status = 'present' THEN 1 ELSE 0 END`)), 'present'],
                [(0, sequelize_1.fn)('COUNT', (0, sequelize_1.col)('AttendanceRecord.id')), 'total'],
            ],
            include: [{
                    model: Models_1.Attendance,
                    as: 'attendance',
                    where: { date: { [sequelize_1.Op.between]: [monthStart.toISODate(), monthEnd.toISODate()] } },
                    include: [{
                            model: index_3.Group,
                            as: "group", // Attendance.belongsTo(Group, { as: ... })
                            where: (0, branch_scope_helper_1.withBranchScope)(req),
